...
const Video = ({
  ...
  const { fetchChannelDetails, currentChannel,fetchWatchLaterVideos } = useChannelState();
  ...
  const newOptions = [
    ...VideoOptions,
    {
      ...
      onClick: async (videoUid) => {
        const {data} = await supabase.from('channelInfo').select().eq('channelRef', currentChannel.uid);
        let videosUid = data[0]?.watchLater;
        const isSaved = videosUid?.includes(videoUid);

        if(isSaved){
          console.log('removing')
          const index = videosUid.indexOf(currentChannel?.uid);
          videosUid.splice(index, 1);
          await supabase.from('channelInfo').update({watchLater: videosUid}).eq('channelRef', currentChannel?.uid);
        }
...
const WatchLaterVideos = () => {
  const { watchLaterVideos } = useChannelState();
